import pygame
import sys
import traceback   #直接打开会显示错误代码，不会立即消失
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()#这样F5点退出就可以了
                sys.exit()

            # 游戏失败
            elif event.type == GAMEOVER:
                loser_sound.play()
                pygame.time.delay(2000)#2000毫秒=2s 单位毫秒
                laugh_sound.play()
                running = False
            ...
            # +=1是加速度
                ...
                #if event.key == K_SPACE:不设需要按空格，轻松点
                # 判断小球是否在坑内
                for each in group:
                    if each.control:
                        for i in hole:   
                            if i[0]-bug <= each.rect.left <= i[1]+bug and \
                               i[2]-bug <= each.rect.top <= i[3]+bug:
                                # 播放音效
                                hole_sound.play()
                                each.speed = [0, 0]
        ...
        for each in balls:
            each.move()
            if each.collide: #碰撞到随机速度弹开 增加难度 后续添加难度选择
